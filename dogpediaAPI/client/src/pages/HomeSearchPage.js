...
class HomeSearchPage extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      breedQuery: window.location.search ? window.location.search.substring(1).split('=')[1] : 'terrier',
      breedResults: null,
      pageNumber: 1,
      pageSize: 10,
      pagination: null  
  ...
  feelingDoggy(){
    getRandomBreed(null, null).then(res => {
      this.breedOnChange(res.breed)
    })
  }
  ...
  componentDidMount() {
    getBreedSearch(this.state.breedQuery).then(res => {
  ...
  render() {
    console.log(getBreedSearch(this.state.breedQuery))
    console.log(this.state.breedQuery)
    return (
      <div>
        <MenuBar />
        <br></br>
